...
def extract_between_tags(tag: str, string: str, strip: bool = False) -> list[str]:
    print("I AM IN EXTRACT BETWEEN TAGS")
...
def generate_better_prompt_via_meta_prompting(input_prompt):
    meta_prompt = metaprompt.replace("{{TASK}}", input_prompt)
    ...
    print(response_body)
    message = pydash.get(response_body, 'content.0.text')
    ...
    for variable in variables:
        prompt_with_variables = extracted_prompt_template.replace(variable, input_prompt)
        pretty_print(prompt_with_variables)
